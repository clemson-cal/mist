// repl_session.cpp - implementation of repl_session_t
...
repl_session_t::repl_session_t(engine_t& engine, std::ostream& out, std::ostream& err)
    : engine_(engine)
    , out_(out)
    , err_(err)
...
auto repl_session_t::get_next_command() -> std::optional<std::string> {
...
void repl_session_t::switch_to_interactive_mode() {
...
auto repl_session_t::execute_line(const std::string& input) -> bool {
    // Handle script files
...
auto repl_session_t::load_script(const std::string& filename) -> bool {
...
void repl_session_t::show_repeating_commands() {
...
void repl_session_t::format_response(const response_t& r) {
