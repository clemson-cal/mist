...
// =============================================================================
// Serializable concepts - supports both member and ADL fields()
// =============================================================================
...
template<typename T>
concept HasMemberFields = requires(T t) {
    { t.fields() };
};

template<typename T>
concept HasMemberConstFields = requires(const T t) {
    { t.fields() };
};

// Check for ADL fields()
template<typename T>
concept HasAdlFields = requires(T t) {
    { fields(t) };
};

template<typename T>
concept HasAdlConstFields = requires(const T t) {
    { fields(t) };
};

// Unified accessor - prefer ADL, fallback to member
template<typename T>
auto get_fields(T& t) {
    if constexpr (HasAdlFields<T>) {
        return fields(t);
    } else {
        return t.fields();
    }
...
template<typename T>
auto get_fields(const T& t) {
    if constexpr (HasAdlConstFields<T>) {
        return fields(t);
    } else {
        return t.fields();
    }
...
template<typename T>
concept HasFields = detail::HasMemberFields<T> || detail::HasAdlFields<T>;

template<typename T>
concept HasConstFields = detail::HasMemberConstFields<T> || detail::HasAdlConstFields<T>;
...
void serialize(A& ar, const T& value) {
    ar.begin_group();
    std::apply([&ar](auto&&... f) {
        (serialize(ar, f.first, f.second), ...);
    }, detail::get_fields(value));
    ar.end_group();
}
...
auto deserialize(A& ar, T& value) -> bool {
    if (!ar.begin_group()) return false;
    std::apply([&ar](auto&&... f) {
        (deserialize(ar, f.first, f.second), ...);
    }, detail::get_fields(value));
    ar.end_group();
    return true;
}
