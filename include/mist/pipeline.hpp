...
void execute_exchange(
    ...
    using T = typename Stage::value_type;
    constexpr auto S = Stage::rank;

    auto publications = std::vector<array_view_t<const T, S>>{};
...
void execute(
    ...
    std::vector<typename pipeline_t<Stages...>::context_t>& contexts,
    Comm& comm,
    Sched& sched,
    Prof& profiler
) {
